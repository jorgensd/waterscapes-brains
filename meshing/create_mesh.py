...
if __name__ == "__main__":
    ...
    To run the following commands, you need to have FEniCS installed.
    See the README in this repo
    ...
    *Other notes to self and others*
    ...
    parser.add_argument("--create", type=int, default=0, help="Create mesh with given int resolution")
    parser.add_argument("--create_lh", type=int, default=0, help="Create lh mesh with given int resolution")
    parser.add_argument("--postprocess", type=int, default=0, help="Convert meshio data to FEniCS (HDF5) file")
    parser.add_argument("--postprocess_lh", type=int, default=0, help="Convert lh meshio data to FEniCS (HDF5) file")
    parser.add_argument("--stldir", type=str, default="stl-surfaces", help="Directory for storing stl files")
    parser.add_argument("--base", type=str, default="../freesurfer/abby/surf/", help="Relative path to FreeSurfer surfaces")
    ...
    if args.generate_stl:
        ...
        for f in files[:-1]:
            outfile = f +".stl"
            subprocess.run(["mris_convert", args.base + f, "./" + outfile])
            print("Moving %s to %s/..." % (outfile, stldir))
            subprocess.run(["mv", outfile, os.path.join(stldir, outfile)])
